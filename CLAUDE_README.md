...
- **`DataTable.add_row` always appends at the BOTTOM (Bundle-4 newjobs bug, 2026-05-11)** — Textual's `add_row` is documented to add "at the bottom of the DataTable"; it does NOT honor any external iteration sort order. `JobsTable.update_snapshot` / `_apply_filter_async`'s in-place diff (mirror in `NodesTable`) updates existing rows via `update_cell` and adds new IDs via `add_row`, so a new high-ID job under `sort=Job↓` landed at the bottom even though `jobs.sort(...)` placed it at index 0 in the iteration. User-facing symptom: "new jobs don't appear at the top until I toggle a tab" (the tab toggle's `_row_keys.clear()` side-effect forced a full rebuild). Fix: compute `incoming_ids - self._row_keys.keys()` upfront and force the `_applied_sort` clear+rebuild branch when any new ID appears, so the loop re-adds top-down in sort order. Any future diff path that mixes `update_cell` with `add_row` and honors a sort order MUST either detect new IDs and clear+rebuild OR follow up with `table.sort(...)`. Pinned by `tests/test_jobs_table_newrow.py`.
...
- **`AsyncSSHExecutor` has NO keepalive and NO reconnect → long-running boards silently wedge (diagnosed 2026-06-05).** `exec.py` opens ONE persistent `asyncssh` connection per cluster and reuses it forever. Two coupled gaps: (1) `connect()` passes no `keepalive_interval`, and asyncssh's default is **0 = disabled** (the `ServerAliveInterval 60` in `~/.ssh/config` does NOT apply to asyncssh — that's OpenSSH-subprocess-only), so an idle connection through TUM's NAT (~2h idle drop, see `cluster_rohan.md`) gets silently killed; (2) `run()` only opens a connection when `self._conn is None` (set back to None ONLY by `aclose()` at unmount), so a dropped connection is never replaced — every subsequent tick calls `create_process()` on the dead conn, raises, gets swallowed by `_refresh_all`'s `except Exception` (`app.py:397`), and the panels keep their placeholders forever. **Symptom:** both boards stuck on "no node data yet" / "no storage info yet" + frozen last-good values, while the clock + matrix animation keep running (those are the local render loop, independent of SSH). NOT the 2026-05-05 ControlPersist orphan-mux failure mode — asyncssh is in-process, so there are zero ssh subprocesses/muxes to find. **Recovery:** `tmux respawn-pane -k` on each board (fresh process → fresh connection; data returns in <5s). **Permanent fix:** see Known follow-ups. (The matrix-rain katakana in the bottom slack area is the intended `animation = "matrix"` AsciiArt widget — a static capture-pane shows one frame and looks like garbage, but it is NOT a bug.)

## Known follow-ups (planned but not yet implemented)

- **Add keepalive + reconnect to `AsyncSSHExecutor` (`exec.py`)** — fixes the long-uptime silent-wedge gotcha above. Two coupled changes: (1) in `connect()`, pass `keepalive_interval=30, keepalive_count_max=5` to `asyncssh.connect()` so idle connections survive TUM's NAT; (2) in `run()`, treat a dropped connection as recoverable — on a connection-level error (asyncssh `ConnectionLost` / `OSError` / closed-channel when calling `create_process`/`communicate`), reset `self._conn = None` and retry connect+run **once** within the same call, re-raising only if the second attempt also fails. Keep the existing cancellation-safe cleanup. Verify against the installed asyncssh version's exact exception types, and run the Pilot suite + the `lint_sync_call_on_async` guard before merge. Without (1) the connection still dies on >keepalive-less idle; without (2) the very first drop wedges the board until manual respawn.
